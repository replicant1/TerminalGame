...
file, the docstring on each class, and each documented method with the
signature and the docstring it was written with. Generated by walking the AST,
so both are verbatim.

**Ten classes** carry the app: three frozen dataclasses, one abstract base,
and six that do the work. Two exception types make twelve in all. Those two are listed below, beside
...
- **An underlined signature is a `@classmethod`.** UML underlines members that
  ...
  <u><code>Maze._from_rows</code></u> — and the diagram underlines the public
  one of them.
- **A `@property` is an attribute, not an operation.** All five in the app are
  ...
  `maze.rows()` — so the diagram lists the four public ones in its attribute
  compartment and the text below gives each a type rather than a signature.
  Each is still tagged `@property`, since that is what makes an attribute out
  of a method.
- **The diagram carries the public surface.** Anything named with a leading
  underscore stays out of it: private fields, private methods, and private
  module constants. The reference below is the wider of the two — it keeps its
  entry for a method that has since been made private, because a name that was
  part of the surface is still worth being able to look up. So the two no
  longer hold the same set, and the underscores in the text are the tell.
...
## `terminalgame/presentation/ghost.py`
...
> - `position` — The cell the ghost is standing on, in cells rather than characters -- a strategy never sees the screen.
> - `heading` — The step the ghost took to arrive, which is what "carry straight on" means. The first tick of a game has a heading the ghost never actually took, so a strategy must cope with it pointing at a wall.
> - `player` — The cell the player is standing on, for a strategy that hunts.

### `GhostStrategy` — inherits `ABC`

> Chooses which way a ghost goes next. Owns no state of the game's.

#### <code>next_step(surroundings: Surroundings) -> Step</code> · `@abstractmethod`
...
>
> **Args**
> - `surroundings` — Where the ghost is, which way it is facing, and the maze and player to reason about.
>
> **Returns**
...
#### <code>next_step(surroundings: Surroundings) -> Step</code>
...
>
> **Returns**
> - The heading unchanged where the cell ahead is open, otherwise a turn chosen at random, and the way back only when there is nothing else at all.

### `open_steps(surroundings: Surroundings, exclude: Sequence[Step]=()) -> Tuple[Step, ...]`
...
> - `exclude` — Steps to leave out whether they are open or not, which is how a strategy refuses to double back.
>
> **Returns**
