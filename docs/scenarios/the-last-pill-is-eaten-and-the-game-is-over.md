...
|---|---|
| The clock reaches its next deadline and calls [`tick`](../../terminalgame/presentation/view_model.py#L293) | Returns at once. The ghost does not move, the count of ticks does not go up, and no picture is built |
| A player presses an arrow and the loop calls [`on_direction`](../../terminalgame/presentation/view_model.py#L312) | Returns at once. No move, no pill, no score |
...
|---|---|
| [`GameViewModel`](../../terminalgame/presentation/view_model.py#L227) | In this scenario it is the **referee**: [`_take_pill`](../../terminalgame/presentation/view_model.py#L342) counts the pills down, [`on_direction`](../../terminalgame/presentation/view_model.py#L312) notices the count has reached nothing, and [`_status_line`](../../terminalgame/presentation/view_model.py#L417) writes a different line from then on |
...
| 2 | [`on_direction`](../../terminalgame/presentation/view_model.py#L312)`(1, 0)` | The same call as every other move |
...
| 6 | [`emit`](../../terminalgame/util/flow.py#L56)`(a picture whose readings line says CLEARED)` | The line of readings is built fresh for every picture, and from now on [it takes the other branch](../../terminalgame/presentation/view_model.py#L440): the score, the word `CLEARED`, and a reminder of the key that quits |
