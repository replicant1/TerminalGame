...
| An arrow key into a wall | [Returns before publishing](../../terminalgame/presentation/view_model.py#L325). No picture is built at all, so nothing is offered |
| A key that is neither an arrow nor a quit key | The loop ignores it. The view model is never called |
| Anything at all after the last pill | [Returns immediately](../../terminalgame/presentation/view_model.py#L302), beats included. Nothing is built and nothing is offered |
