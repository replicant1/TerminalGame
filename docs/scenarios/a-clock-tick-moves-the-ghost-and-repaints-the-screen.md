...
| [`GameViewModel`](../../terminalgame/presentation/view_model.py#L227) | Everything the game knows about where things are and what has happened. In this scenario it is the **mover**: [`tick`](../../terminalgame/presentation/view_model.py#L293) advances the ghost and the count, then builds an entirely new picture rather than altering the old one |
| [`SimpleGhostStrategy`](../../terminalgame/presentation/ghost.py#L106) | One implementation of [`GhostStrategy`](../../terminalgame/presentation/ghost.py#L60), and the one the game is built with. In this scenario it is the **navigator**: [`next_step`](../../terminalgame/presentation/ghost.py#L126) is handed the maze, where the ghost is and which way it is facing, and answers with a direction. It moves nothing and remembers nothing between ticks — the view model owns where the ghost stands, which is what lets a different ghost be swapped in without the rules changing |
...
| 9 | [`tick`](../../terminalgame/presentation/view_model.py#L293)`()` | This is the function the clock was handed when it was built. The clock has no idea what it does, and no idea that a game exists. It holds a function and calls it. That is the whole of the arrangement |
