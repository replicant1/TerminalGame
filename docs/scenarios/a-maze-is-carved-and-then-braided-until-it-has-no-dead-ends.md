...
[**Carving and braiding, one step at a time**](https://replicant1.github.io/TerminalGame/docs/step-by-step/maze-step-by-step.html) is the
...
junctions it could reach from there, which one the random number picked, and
which wall that opened. The route back to the start is shaded as it grows and shrinks, and
the dead ends are ringed as they appear, so the count climbing through the carve
and falling to zero through the braid happens in front of you rather than being
asserted here.

It is not an approximation of the algorithm. The page carries a Mersenne Twister
seeded and drawn from exactly the way CPython's `random` module seeds and draws
from its own, so a seed typed into the page gives the maze
[`Maze.generate(11, 11, seed=n)`](../../terminalgame/presentation/maze.py#L60)
gives in Python, cell for cell. **Seed 7 is the maze in the illustration above**,
and the page ends its carve on the same seven dead ends the first panel rings in
red.
...
copy, [replicant1.github.io/TerminalGame/docs/step-by-step/maze-step-by-step.html](https://replicant1.github.io/TerminalGame/docs/step-by-step/maze-step-by-step.html),
...
limitation of that viewer rather than of the file. `docs/step-by-step/maze-step-by-step.html`
...
[every wall cell picks its glyph](https://replicant1.github.io/TerminalGame/docs/step-by-step/wall-glyphs-step-by-step.html) turns the
finished maze into the two layers a picture is drawn from, cell by cell. Seed 7
is the same maze in both.

| Class | What it represents, and its part in this scenario |
|---|---|
| [`GameViewModel`](../../terminalgame/presentation/view_model.py#L227) | Everything the game knows about where things are. In this scenario it is only the **customer**: it asks for a maze of a particular size during its own construction, and everything it does afterwards — placing the player and the ghost, deciding whether a move is allowed — is a question put to the maze it was given |
...
sequenceDiagram
    ...
    Maze->>Maze: walks to an unvisited junction and opens the wall between
