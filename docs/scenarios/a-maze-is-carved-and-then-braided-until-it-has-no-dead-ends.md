...
| 6 | [braids](../../terminalgame/presentation/maze.py#L335), opening a second exit for every dead end | The second pass. A dead end is a junction with only one way out: of the four wall cells beside it, one has been opened and the rest are still closed. The fix is to open a second. Every junction has at least two other junctions two cells away from it, even one in the corner of the grid, so a second exit can always be found — which is what makes "no dead ends" a promise rather than an attempt |
