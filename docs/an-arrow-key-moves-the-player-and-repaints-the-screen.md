...
One more detail is worth stating before the diagram, because it explains a step
that would otherwise look like fussiness. The player's new position is not
simply the old position plus one. It is the old position plus one, brought back
inside the arena walls if that would have taken it outside them. The arena is
drawn with a border, so the character has to stay between
[row 1 and row 27, and between column 1 and column 78](../terminalgame/presentation/view_model.py#L89),
which was confirmed by running the program. Without that, a player holding down
an arrow key would walk the character straight through the wall and off the
edge of the picture.
...
| [`GameScreen`](../terminalgame/ui/screen.py#L45) | The only part of the program that knows anything about curses[^curses]. In this scenario it does two separate jobs at opposite ends of the story. First it is the **ear**: [`read_key`](../terminalgame/ui/screen.py#L137) hands back one plain number and attaches no meaning to it. Later it is the **painter**: [`render`](../terminalgame/ui/screen.py#L149) draws the finished picture, and it is called without having asked for anything |
| [`GameViewModel`](../terminalgame/presentation/view_model.py#L30) | Everything the game knows about where things are. In this scenario it is the **mover**: [`on_direction`](../terminalgame/presentation/view_model.py#L56) applies the step, keeps the result inside the walls, and builds an entirely new picture. It is told a direction and never learns what caused it |
...
sequenceDiagram
    ...
    ViewModel->>ViewModel: adds one to the player column and keeps it inside the walls
    ...
    Curses->>Curses: writes 47 bytes to the terminal
```
...
| 2 | [`read_key`](../terminalgame/ui/screen.py#L137)`()` | The loop asks for a key on every single pass. Most of the time there is nothing to collect, and the answer is nothing at all. This particular pass is one of the rare ones where a player has actually pressed something |
...
| 8 | [`on_direction`](../terminalgame/presentation/view_model.py#L56)`(0, 1)` | **This is the moment the key stops being a key.** What crosses over is the pair of numbers zero and one, meaning no change to the row and one more column. The view model[^viewmodel] is never told which key was pressed, or that a key was pressed at all. The same call could just as easily come from a test, or later from a game controller, and nothing on the far side would need changing |
| 9 | adds one to the player column and keeps it inside the walls | Two things in one step. The step is applied, and then the result is brought back inside the arena if it fell outside. The permitted area runs from row 1 to row 27, and from column 1 to column 78. Those come from the size of the playfield[^playfield] with the border taken off, so the character can never be drawn on top of a wall |
...
| 16 | writes 47 bytes to the terminal | Measured, not estimated. The game was run inside a false terminal of exactly 30 rows by 80 columns, arrow keys were sent to it, and the bytes written in response to each one were counted. Forty-seven bytes covers three separate things: putting an ordinary arena character back where the player used to be, drawing the player in its new place, and rewriting the part of the readings line that names the position. Some measured bursts came to 93 bytes, which is this repaint and a clock repaint arriving too close together to tell apart, and 47 plus 46 is 93 |
...
rather than running past it, because every step is brought back inside the
permitted area. And once it has stopped, each further press produces a picture
identical to the one on screen, which the carrier drops before any bytes are
written. So a player leaning on an arrow key against a wall costs the terminal
nothing at all.
...
[^frozen]: A **frozen** value is one that cannot be altered after it is made.
    ...
    [`ViewState`](../terminalgame/presentation/state.py#L35) and
    [`Sprite`](../terminalgame/presentation/state.py#L25) are frozen. Two
