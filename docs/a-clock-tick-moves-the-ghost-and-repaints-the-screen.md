...
Every tick builds a full picture, 30 rows by 80 columns, and hands it over as
...
| [`GameViewModel`](../terminalgame/presentation/view_model.py#L30) | Everything the game knows about where things are and what has happened. In this scenario it is the **mover**: [`tick`](../terminalgame/presentation/view_model.py#L50) advances the ghost and the count, then builds an entirely new picture rather than altering the old one |
...
| [`GameScreen`](../terminalgame/ui/screen.py#L45) | The only part of the program that knows anything about curses[^curses]. In this scenario it is the **painter**: [`render`](../terminalgame/ui/screen.py#L149) is called because it subscribed once, long ago, and it has asked for nothing since |
...
sequenceDiagram
    ...
    Curses->>Curses: writes about 46 bytes to the terminal
    Clock-->>Main: one tick fired
```

| Step | Message | What is going on |
|---:|---|---|
| 1 | [`read_key`](../terminalgame/ui/screen.py#L137)`()` | The loop always reads a key first and asks about the clock second. The reading is what makes the loop pause rather than spinning at full speed and heating the machine up for nothing |
...
| 5 | [`read_key`](../terminalgame/ui/screen.py#L137)`()` | Round again. The two wasted passes drawn here stand for however many there really are |
...
| 18 | writes about 46 bytes to the terminal | Measured, not estimated. The game was run inside a false terminal of exactly 30 rows by 80 columns, and the bytes written for each frame were counted. Steady frames were between 43 and 47 bytes, and almost always 46. Set that against the 5,214 bytes the very first picture costs, and against the 2,400 character positions the picture describes. Working out what changed by hand, inside the view model, would add a great deal of bookkeeping and could not possibly do better than 46 bytes |
